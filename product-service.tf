...
[
  {
    ...
    "cpu": 256,
    "memory": 300,
    "image": "docker.io/docker131186/product-service:latest",
    "essential": true,
    "name": "product-service",
    "logConfiguration": {
    "logDriver": "awslogs",
      "options": {
        "awslogs-group": "/aws-ecs-log/product-service",
...
resource "aws_cloudwatch_log_group" "product-service-log-group" {
  name = "/aws-ecs-log/product-service"
}
...
resource "aws_alb_listener" "aws-dev-alb-listener-port-product-service" {
